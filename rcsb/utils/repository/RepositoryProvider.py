...
#   24-Apr-2025   bv Add a fix to handle merging of duplicated and partially populated data categories in mmCIF validation reports
##
"""
...
class RepositoryProvider(object):
            ...
            #
                ...
                if cL:
                    for dD in locatorObj[1:]:
                        ...
                        for mc in mergeL:
                            # Interim fix for handling duplicated and partially populated data categories in vrpt mmcif files
    ...
    def __getEntryUriList(self, idCodeList=None, mergeContentTypes=None):
        ...
        try:
            ...
            if not (mergeContentTypes and "vrpt" in mergeContentTypes):
                logger.warning("Validation mergeContentTypes not enabled!")
            #
            for tId in tIdL:
                kwD = HashableDict({})
    ...
    def __getCompModelPathList(self, idCodeList=None, fmt="mmcif"):
        return self.__fetchModelPathList(self.__baseUrlCSM, idCodeList=idCodeList, fmt=fmt, numProc=self.__numProc)
